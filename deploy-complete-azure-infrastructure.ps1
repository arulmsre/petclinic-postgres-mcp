...
try {
    ...
    if (-not $SkipResourceCreation) {
        ...
        $postgresExists = az postgres flexible-server show --name $PostgresServerName --resource-group $ResourceGroupName 2>$null
        if ($postgresExists) {
    ...
    if (-not $SkipDeploy) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Failed to deploy PetClinic application"
        }
...
Write-Host 'Prometheus MCP Port-Forward (8090:80)' -ForegroundColor Green
kubectl port-forward -n petclinic service/prometheus-mcp-service 8090:80
"@
        
        Start-Process powershell -ArgumentList "-NoExit", "-Command", @"
Write-Host 'Grafana MCP Port-Forward (8091:80)' -ForegroundColor Yellow
kubectl port-forward -n petclinic service/grafana-mcp-service 8091:80
"@
        
        Start-Process powershell -ArgumentList "-NoExit", "-Command", @"
Write-Host 'PostgreSQL MCP Port-Forward (8092:80)' -ForegroundColor Cyan
