#!/usr/bin/env node
/**
 * Grafana MCP HTTP-to-stdio Adapter
 * 
...
const MCP_HTTP_URL = 'http://localhost:8091';
...
function log(message) {
  console.error(`[Grafana Adapter] ${new Date().toISOString()} - ${message}`);
}
  ...
  try {
    ...
    return await new Promise((resolve, reject) => {
      ...
      req.on('error', (err) => {
        ...
        log(`Make sure the Grafana MCP HTTP server is running on ${MCP_HTTP_URL}`);
        resolve([]); // Return empty array instead of rejecting
      });
      req.on('timeout', () => {
        req.destroy();
  ...
  try {
    ...
    const errorResponse = {
      ...
      error: {
        ...
        data: { adapter: 'grafana-http-adapter' }
      },
      id: request?.id || null
    };
...
// Startup message
log('Grafana MCP HTTP-to-stdio adapter started');
